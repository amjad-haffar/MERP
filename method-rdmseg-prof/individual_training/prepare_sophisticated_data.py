...
def main(source_dir: Path, output_dir: Path) -> None:
    # Load the cleaned MERP annotations and profiles
    ...
    # Keep one profile row per listener
    music_profiles = (
        profiles[
            ["workerid"] + MUSIC_FEATURES
        ]
        .drop_duplicates(subset="workerid")
    )

    # Attach the profile features to each individual annotation
    ...
    # Store the three values as one profile vector
    data["profile"] = data[MUSIC_FEATURES].apply(
    ...
    print(
        ...
        data[MUSIC_FEATURES].isna().sum().to_dict(),
    )
