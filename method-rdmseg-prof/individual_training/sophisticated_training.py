...
if __name__ == "__main__":
    ...
    if args.affect_type == "valences":
        exps = pd.read_pickle(
            os.path.join(
                DATA_DIR,
                "exps_individual_valence_sophisticated.pkl",
            )
        )
        ...
        exps = pd.read_pickle(
            os.path.join(
                DATA_DIR,
                "exps_individual_arousal_sophisticated.pkl",
            )
        )
    else:
        raise ValueError(
    ...
    def dataloader_prep(
        ...
        # Convert the profile column into a matrix:
        # rows = annotations
        # columns = instrument, training, duration
        profile_matrix = np.stack(
            split_exps["profile"].to_numpy()
        ).astype(np.float32)

        # Fit standardisation using the training fold only
        if profile_mean is None:
            profile_mean = profile_matrix.mean(axis=0)
            profile_std = profile_matrix.std(axis=0)

            # Avoid division by zero if a feature is constant
            profile_std = np.where(
                profile_std == 0,
                1.0,
                profile_std,
            )

        standardised_profiles = (
            profile_matrix - profile_mean
        ) / profile_std

        # Put each three-value vector back into the dataframe
        split_exps["profile"] = list(
            standardised_profiles.astype(np.float32)
        )

        params = {
            "shuffle": not test,
            "num_workers": args.num_workers,
            "batch_size": 1 if test else args.batch_size,
        }

        seq_len = None if test else args.num_timesteps
        ...
        return loader, profile_mean, profile_std
    ...
    for fold_i in range(num_folds):
        ...
        train_feat_dict = util.load_pickle(
            os.path.join(DATA_DIR, "folds", f"train_feats_{fold_i}.pkl")
        )

        test_feat_dict = util.load_pickle(
            os.path.join(DATA_DIR, "folds", f"test_feats_{fold_i}.pkl")
        )
        
        train_loader, profile_mean, profile_std = dataloader_prep(
            train_feat_dict,
            exps,
            args,
            test=False,
