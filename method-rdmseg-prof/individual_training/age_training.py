...
def train(train_loader, model, valid_loader, args):
    ...
    plt.ylim([-1, 1])
    plt.legend()
    plt.title(f"Loss || init mse:{loss_log['train_mse'][0]:.3f} | r:{loss_log['train_r'][0]:.3f} || test mse: {best_valid_ave_mse:.3f} | r: {best_valid_ave_r:.3f}")
...
if __name__ == "__main__":
    ...
    if args.affect_type == "valences":
        exps = pd.read_pickle("data/exps_individual_valence_age.pkl")
    elif args.affect_type == "arousals":
        exps = pd.read_pickle("data/exps_individual_arousal_age.pkl")
    else:
    ...
    def dataloader_prep(feat_dict, exps, args, mean=None, std=None):

        # Keep only annotations belonging to songs in this split
        songlist = list(feat_dict.keys())
        split_exps = exps[exps["songurl"].isin(songlist)].copy()

        # Compute statistics from the TRAIN split only
        if mean is None:
            mean = split_exps["profile"].mean()
            std = split_exps["profile"].std()

        # Standardise age
        split_exps["profile"] = (split_exps["profile"] - mean) / std
