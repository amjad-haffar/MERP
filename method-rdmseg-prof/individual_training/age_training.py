...
if __name__ == "__main__":
    ...
    if args.affect_type == "valences":
        exps = pd.read_pickle(
            os.path.join(DATA_DIR, "exps_individual_valence_age.pkl")
        )
    elif args.affect_type == "arousals":
        exps = pd.read_pickle(
            os.path.join(DATA_DIR, "exps_individual_arousal_age.pkl")
        )
    else:
    ...
    def dataloader_prep(
        ...
        if profile_mean is None:
            profile_mean = split_exps["profile"].mean()
            profile_std = split_exps["profile"].std()

        split_exps["profile"] = (
            split_exps["profile"] - profile_mean
        ) / profile_std
